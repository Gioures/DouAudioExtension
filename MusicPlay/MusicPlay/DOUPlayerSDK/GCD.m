...
@property (nonatomic, strong) dispatch_queue_t searialQueue;
...
/**
 GCD取消操作的block必须是 dispatch_block_create 创建的block,因此要在这里转换一下下

 @param afterBlock GCD事件block
 */
-(void)setAfterBlock:(void (^)(void))afterBlock{
    dispatch_block_t b = dispatch_block_create(0, afterBlock);
    _afterBlock = b;
}
...
-(dispatch_queue_t)searialQueue{
    if (!_searialQueue) {
        _searialQueue = dispatch_queue_create("com.hmc.www", DISPATCH_QUEUE_SERIAL);
    }
    return _searialQueue;
}

-(GCD * _Nonnull (^)(void))enter{
    return ^{
...
-(GCD * _Nonnull (^)(void (^ _Nonnull)(void)))groupAsyn{
    return ^(void(^block)(void)){
        self.enter();
        dispatch_group_async(self.group, self.searialQueue, block);
        return self;
    };
}

-(GCD * _Nonnull (^)(void (^ _Nonnull)(void)))groupNotify{
    return ^(void(^block)(void)){
        dispatch_group_notify(self.group, self.searialQueue, block);
        return self;
    };
}

-(GCD*  _Nonnull (^)(void (^ _Nonnull)(void)))main{
    return ^(void(^block)(void)){
...
-(GCD*  _Nonnull (^)(void (^ _Nonnull)(void)))globle{
    return ^(void(^block)(void)){
...
-(GCD*  _Nonnull (^)(float, void (^ _Nonnull)(void)))after{
    return ^(float time , void(^block)(void)){
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(time * NSEC_PER_SEC)), dispatch_get_main_queue(), block);
        return self;
    };
}

-(GCD * _Nonnull (^)(dispatch_block_t _Nonnull))cancelAfter{
    return ^(void(^block)(void)){
...
void apply(size_t time , void(^block)(size_t)){
    dispatch_apply(time, dispatch_get_global_queue(0, 0), block);
}
